    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
        <motion.div
          ...
          className="glass-panel w-full max-w-xl rounded-3xl p-6 lg:p-8 shadow-2xl border border-slate-700/80 relative overflow-hidden space-y-6 max-h-[90vh] overflow-y-auto"
        >
          {/* Close button */}
          <button
          ...
          <div className="p-4 rounded-2xl bg-slate-800/40 border border-slate-700/60 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-400">From Station</span>
              ...
              <div className="text-right">
                <span className="text-[10px] uppercase font-bold text-slate-400">To Station</span>
                <p className="text-sm font-bold text-slate-100 flex items-center gap-1.5 mt-0.5 justify-end">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
            <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Travel Class</span>
              <p className="font-bold text-slate-100 mt-1">{b.travel_class || "N/A"}</p>
            </div>

            <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Quota</span>
              <p className="font-bold text-slate-100 mt-1">{b.quota || "GENERAL"}</p>
            </div>

            <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50 col-span-2 sm:col-span-1">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Train Number</span>
              <p className="font-bold text-slate-100 mt-1">#{b.train_number}</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-indigo-950/20 border border-indigo-500/20 space-y-2 text-xs">
            ...
            <div className="pt-2 border-t border-indigo-500/30 flex items-center justify-between text-sm font-bold text-slate-100">
              <span>Total Fare Charged</span>
              <span className="text-emerald-400 text-base">{formatCurrency(b.total_fare, 2)}</span>
            </div>
          </div>
        </motion.div>
      </div>
